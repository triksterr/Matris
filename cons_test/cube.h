...
private:
	...
	//! В слоях - смещение = 0 ??? Вряд ли оно будет использоваться
	int digit; // число в кубике
	RGBcolor color; // цвет
	int type; // тип кубика: обычный = 0, шарик (*), пушка (=||=), ракетка (===), (вращающийся кубик???)...
	int status; // статус кубика: 0 - одиночный (?), 1 - в фигуре, 2 - в слоях
...
public:
	...
	Cube() : Cube(0, 0, 0, RGBcolor(255, 255, 255), 0) // Конструктор из фигуры - при создании кубика в фигуре?
	{
	...
	// деструктор
	~Cube() = default;
	//! записать в фигуру или в слои nullptr!
	...
	bool moveL(const int shift = 1)  
	{
		...
		//! Проверяем контакт с кубиками нижних слоев!
		return true;
	}
	...
	{ 
		...
		//! Здесь тоже надо проверять контакт!
		//! 
		//! *******
		//! *
		//! **  ***
	...
	// Проверка каcания кубиком стакана

	// Проверка вылета кубика за стакан
